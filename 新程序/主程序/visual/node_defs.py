...
NODE_DEFS: dict[str, dict] = {
    ...
    "scene_probe": {
        ...
        "params": [
            ...
            {"name": "accuracy", "label": "精度(0=全部)", "widget": "spinbox",
             "default": 0, "min": 0, "max": 99},
            {"name": "timeout", "label": "判定次数", "widget": "spinbox", "default": 3,
             "min": 1, "max": 60},
            {"name": "output_var", "label": "输出变量", "widget": "text", "default": ""},
        ],
        "description": "判定当前界面=示教场景（红框搜索范围+蓝框整体标识）；命中蓝框数>=精度(0=全部)则通过；最多判定 N 次；命中写 1/未命中写 0 到输出变量",
    },
    "matcher": {
        ...
        "params": [
            {"name": "template", "label": "目标元素", "widget": "combo_element",
             "default": ""},
            {"name": "region", "label": "搜索区域", "widget": "text", "default": ""},
            ...
            {"name": "timeout", "label": "判定次数", "widget": "spinbox",
             "default": 3, "min": 1, "max": 60},
        ],
        "description": "识别某元素；命中走 out（输出坐标），未命中走 miss；index 选第 N 个同名匹配；搜索区域为空=全屏",
    },
    "ocr_reader": {
    ...
    "clicker": {
        ...
        "description": "固定点：点示教点；示教点若为框选区域→区域内随机点击；随机点：屏幕随机；识别坐标：点击上游识图器输出的坐标中心",
    },
    "dragger": {
