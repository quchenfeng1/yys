...
w = n.get_widget("point")   # NodeComboBox（点击目标）
group = w.widget()
lay = group.layout()
...
assert item0.text() == "点击目标"
print("  ✅ label 在左、输入在右")

print("[2] 水平布局下参数仍可读写/导出")
w.set_value(list(["pt.a", "pt.b"]))
w.set_value("pt.a")
task = c.export_task({"graph": {"nodes": [], "connections": []}, "teach": {}})
clicker = next(x for x in task["graph"]["nodes"] if x["type"] == "clicker")
print("  导出 point:", clicker["params"].get("point"))
assert clicker["params"].get("point") == "pt.a"
print("  ✅ 参数导出正常")

print("[3] 其他 widget 类型同样水平（spinbox/text/checkbox）")
n2 = c.add_node("scene_probe")
w2 = n2.get_widget("timeout")  # spinbox
assert isinstance(w2.widget().layout(), QHBoxLayout)
n3 = c.add_node("dragger")
w3 = n3.get_widget("direction")  # combo
