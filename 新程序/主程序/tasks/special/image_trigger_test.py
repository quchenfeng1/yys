"""
识图触发测试任务（trigger 类型：识别到特定图片自动触发）。

触发方式（说明书 02 §3.6 TriggerWatcher + 05 §3.1 trigger 例外）：
  02-TriggerWatcher 持续监控本任务的 trigger_templates 识别列表
  → 任一模板识别命中 → 发布 trigger_detected 事件
  → 05-Scheduler 订阅后 update_next_run(name, now) → 入队执行
...
"""
display_name = "识图触发测试"
description = "trigger 识图触发：识别到 trigger_templates 图片自动执行一次"
...
class TaskStart(TaskStep):
    ...
    def execute(self, context=None):
        ...
        _log("▶ 步骤1/4 任务开始：由 TriggerWatcher 识别到 trigger_templates 自动触发（trigger 识图触发）",
...
class ImageTriggerTestTask(BaseTask):
    """识图触发测试任务入口（task_id 必须与 tasks.yaml 的 name 一致）"""
    task_id = "image_trigger_test"
    category = "special"
