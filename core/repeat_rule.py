...
class RepeatRule:
    ...
    at_time: Optional[str] = None                # "HH:MM"（daily/weekly/monthly/interval_days 用）
    weekdays: Optional[list[int]] = None          # [1,3,5]（1=周一...7=周日）
    month_days: Optional[list[int]] = None        # [1,15]
    ...
    window: Optional[dict] = None                # 可选时间窗口 {start:"17:00", end:"22:00"}
    ...
    def validate(self) -> list[str]:
        """校验规则合法性，返回错误信息列表。"""
        errors = []
        valid_types = {"once", "daily", "weekly", "monthly", "interval_days", "interval_hours", "expire_at"}
        if self.type not in valid_types:
            errors.append(f"无效的 repeat.type: {self.type}，应为 {valid_types}")
        if self.type in ("daily", "weekly", "monthly", "interval_days") and not self.at_time:
            errors.append(f"{self.type} 类型必须指定 at_time")
        if self.type == "weekly" and not self.weekdays:
            errors.append("weekly 类型必须指定 weekdays")
        if self.type == "monthly" and not self.month_days:
            errors.append("monthly 类型必须指定 month_days")
    ...
    def _next_daily(self, now: datetime) -> datetime:
        target = self._parse_time_today(now, self.at_time or "00:00")
        if target <= now:
    ...
    def _next_weekly(self, now: datetime) -> datetime:
        ...
        current_wday = now.isoweekday()  # 1=Mon ... 7=Sun
        weekdays = sorted(self.weekdays or [1])
        # 找今天或之后最近的匹配日
        for wd in weekdays:
            if wd >= current_wday:
                days_ahead = wd - current_wday
                candidate = now.replace(hour=target_time.hour, minute=target_time.minute, second=0) + timedelta(days=days_ahead)
                if candidate > now:
                    return candidate
        # 下周第一个
        days_ahead = 7 - current_wday + weekdays[0]
        return now.replace(hour=target_time.hour, minute=target_time.minute, second=0) + timedelta(days=days_ahead)
    ...
    @staticmethod
    def _parse_time(now: datetime, time_str: str) -> datetime:
        h, m = map(int, time_str.split(":"))
        return now.replace(hour=h, minute=m, second=0, microsecond=0)
    ...
    def to_dict(self) -> dict:
        d = {"type": self.type, "times": self.times}
        if self.at_time:
            d["at_time"] = self.at_time
        if self.weekdays:
            d["weekdays"] = self.weekdays
        if self.month_days:
            d["month_days"] = self.month_days
        if self.days:
            d["days"] = self.days
        if self.hours:
            d["hours"] = self.hours
