...
| `monitor` | 运行监控 | 占位（尚未实现） |
| `sub_account` | 小号设置 | 占位（尚未实现） |
...
| 状态 | 启动按钮 | 停止按钮 | 暂停按钮 | 状态文字 |
|------|---------|---------|---------|---------|
| 就绪 | ▶ 启动 (绿) | ■ 停止 (灰/禁用) | ⏸ 暂停 (灰/禁用) | ● 就绪 (灰) |
| 运行中 | ▶ 运行中 (灰/禁用) | ■ 停止 (红) | ⏸ 暂停 (黄) | ● 运行中 (蓝) |
| 已暂停 | ▶ 继续 (蓝) | ■ 停止 (红) | ⏸ 暂停 (灰/禁用) | ⏸ 已暂停 (黄) |
...
MainWindow (11-用户界面)
  │
  ├── 直接持有 ──→ ConfigManager (06)    ← 读写配置
  ├── 直接持有 ──→ TaskManager (13)      ← 扫描/创建/删除任务
  ├── 直接持有 ──→ ImageManager          ← 素材管理
  ├── 直接持有 ──→ Scheduler (05)        ← 日程表 + 注入到 ScriptWorker
  │
  ├── 创建 ──→ ScriptWorker (09, QThread)
  │              ├── 注入 Scheduler
  │              ├── 创建 Executor (14) → Recognizer (02) + AntiDetect (03) + ADB (01)
  │              └── 创建 LoginFlow (16)
  │
  ├── 子面板 ──→ ControlBar / TaskQueuePanel / LogPanel / StatusBar
  ├── 可切换 ──→ ConfigPanel / ImageManagerPanel / TaskManagerPanel / 游戏任务面板
  │
  └── 订阅 ──→ EventBus (08) ← 所有模块发布事件
```
